...
- **配置简化**: 语音服务不再使用环境变量，也不依赖 YAML；v1.0 起 ASR/TTS 改为**通用语音平台配置**，在「设置 → 语音服务」页分别选择平台并填写 API 参数，落库 `voice_platform_config`（重启保留、支持连通性测试），DashScope（Qwen3）为默认实现。`QwenAsrService` / `QwenTtsService` 通过 `applyRuntimeConfig` 注入运行态平台配置。
