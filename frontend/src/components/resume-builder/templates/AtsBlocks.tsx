 ...
 * ATS 模板的「分页友好块」版本。
 *
 * 与 AtsTemplate.tsx 视觉一致（单栏极简、无装饰、ATS 友好），但把内容拆成 PageBlock[]，
 * 供 A4Preview 做块级分页：标题与下一条目同页、正文可跨页但不切词。
 * 块与块之间用内边距分隔，保证「测量高度 = 展示高度」。
 *
 * 与 Developer 的差异：
 * - 头部居中
 * - bullet 标记使用 `-`
...
function SectionTitle({ text, pad }: { text: string; pad: number }) {
  return (
    <div style={st({ display: 'flex', alignItems: 'baseline', gap: 6, paddingBottom: 3, marginTop: pad })}>
...
/** 生成 ATS 模板的分页块（视觉与 AtsTemplate 一致） */
export function buildAtsBlocks(doc: ResumeDocument): PageBlock[] {
  const b = doc.basics;
  const blocks: PageBlock[] = [];
  const key = (p: string) => `ats-${p}`;
  ...
  blocks.push({
    ...
    node: (
      <div style={st({ textAlign: 'center', paddingBottom: 12 })}>
        {b.name ? (
          <h1 style={{ margin: 0, fontSize: FONT.name, fontWeight: 700 }}>{b.name}</h1>
        ) : null}
        {b.title ? (
          <p style={{ margin: '4px 0 0', fontSize: FONT.headline, fontWeight: 600 }}>{b.title}</p>
        ) : null}
        ...
        {b.summary ? (
          <p style={{ margin: '10px 0 0', fontSize: FONT.body, lineHeight: LINE }}>{b.summary}</p>
        ) : null}
      </div>
    ),
  });
  ...
  if (doc.experience.length > 0) {
    blocks.push({ key: key('exp-title'), node: <SectionTitle text="Work Experience / 工作经历" pad={16} />, avoid: 'after' });
  ...
  if (doc.projects.length > 0) {
    blocks.push({ key: key('prj-title'), node: <SectionTitle text="Projects / 项目经历" pad={14} />, avoid: 'after' });
  ...
  if (doc.skills.length > 0) {
    blocks.push({ key: key('skill-title'), node: <SectionTitle text="Skills / 专业技能" pad={14} />, avoid: 'after' });
  ...
  if (doc.education.length > 0) {
    blocks.push({ key: key('edu-title'), node: <SectionTitle text="Education / 教育背景" pad={14} />, avoid: 'after' });
  ...
  if (hasCertsAwardsLangs) {
    blocks.push({ key: key('cert-title'), node: <SectionTitle text="Certifications / 证书 / 获奖 / 语言" pad={14} />, avoid: 'after' });
    doc.certifications.forEach((c) =>
      blocks.push({
        key: key(`cert-${c.id}`),
        node: (
          <p style={st({ margin: '6px 0 0', fontSize: FONT.minor, lineHeight: LINE })}>
            {c.name}
            {c.date ? <span style={{ color: MUTED }}> — {c.date}</span> : null}
          </p>
        ),
      }),
    );
    doc.awards.forEach((a) =>
      blocks.push({
        ...
        node: (
          <p style={st({ margin: '6px 0 0', fontSize: FONT.minor, lineHeight: LINE })}>
            {a.title}
            {a.date ? <span style={{ color: MUTED }}> — {a.date}</span> : null}
          </p>
        ),
      }),
    );
  ...
  if (doc.customSections.length > 0) {
    blocks.push({ key: key('other-title'), node: <SectionTitle text="Other / 其他内容" pad={14} />, avoid: 'after' });
